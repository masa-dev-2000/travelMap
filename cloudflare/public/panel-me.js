...
async function bootstrap(){
  ...
  const muteBox=$('#mute-settings');if(muteBox)try{const muteData=await api('mutes');muteBox.replaceChildren();for(const person of muteData.users){const label=el('label',{className:'check'}),input=el('input',{type:'checkbox',checked:!!person.muted});input.onchange=async()=>{try{await api('mutes',{handle:person.handle,muted:input.checked});everyone.reload();}catch(error){input.checked=!input.checked;notify(error.message);}};label.append(input,document.createTextNode(' '+person.display_name+'をミュート'));muteBox.append(label);}}catch{muteBox.textContent='ミュート設定を取得できません';}
