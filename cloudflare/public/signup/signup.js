...
let user={display_name:'',handle:'',avatar_url:null},iconBlob=null,iconPreview=null,busy=false;
...
function paintFace(){
  const face=$('#face'),initial=()=>{face.replaceChildren();face.textContent=[...($('#display-name').value.trim()||'?')][0].toUpperCase();};
  ...
  if(iconPreview)picture(iconPreview);else if(user.avatar_url)picture(user.avatar_url);else initial();
}
...
$('#display-name').oninput=paintFace;
$('#to-3').onclick=()=>{
...
try{
  ...
  $('#display-name').value=user.display_name||'';$('#handle').value=user.handle||'';
