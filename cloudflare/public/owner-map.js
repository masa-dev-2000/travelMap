...
export function makeOwnerMap({settingsSlot}={}){
  ...
  control.append(title,options,status,retry);(settingsSlot||document.querySelector('#map-style-settings')).append(control);control.open=true;
  ...
  async function switchStyle(next){
    ...
    try{
      ...
      document.body.dataset.basemap=mode;title.textContent='地図';title.setAttribute('aria-label','地図の種類: '+STYLES[mode]);control.open=true;
