...
`51853f77-f5bc-4ef8-9b63-9c01abefcfc7`、配備元コードはPR #21のmerge commit `47d2cd2`。

## 画面の分割（Issue #26、2026-09-24）

`profile` の引き出し1枚に、人物情報・記録一覧・収支・旅・分類の5つが同居していた。行き先を分けた。

| 画面 | 入口 | 戻り先 |
|---|---|---|
| プロフィール | 下部ナビ | — |
| 記録の一覧（`timeline`） | 地図の件数表示 `#map-count`（`button` 化） | 閉じる＝地図 |
| おかね（`money`） | プロフィールのリンク | 「‹ 戻る」→ プロフィール（閉じても残る sticky） |
| 旅（`trips`） | プロフィールのリンク | 同上 |
| 分類の追加 | 設定ダイアログ | — |

下部ナビは3つのまま。記録一覧の入口は件数表示だけで、プロフィールには置かない。`map-shell.js` に「開かずに登録する」`place()` と、閉じても消えない戻り先（sticky）、`active()` を足した。

**再生が一時停止でなく破棄される原因が判明した。** `story.js` の `viewchange` が、どの画面を開いても無条件に `finish()` していた。許可リスト（`playback-options` / `record` / `route` / `story` / `timeline`）を `makeStory` に渡し、読むための画面では捨てない。台帳（おかね・旅）と設定では従来どおり止まる。あわせて、次の人物へ移るとき `player.load()` → `begin()` → `shell.hide()` を通るため読んでいる画面が勝手に閉じていた問題も塞いだ（`READING` に含まれる画面では閉じない）。

逆向きレビューで見つけた自作の退行4件を同じPRで修正した。区間から記録の詳細へ入ると戻り先が消える（区間番号を後から読んでおり `rows[null]` で例外）、件数の「›」が閲覧側の再描画で消える（`::after` へ移した）、収支が別画面の絞り込みで変わるのに理由が出ない、閉じたときのフォーカス落ち。

- 配備元は main の merge commit `31b60e9`（PR #43）。マージ直前 head `84ff483` の CI run `35956641968` が core/browser とも成功。ローカルでも型検査・Nodeテスト83件・ビルド・`node --check public/*.js`・通常Chromium45件・実MapLibre4件・移行統合・索引回帰を再確認した。
- Worker Version `74252fdc-9a65-4271-8952-cf7fd0b37f0d` を配備。復旧先は `d1390732-7e08-4197-94f0-af0bee0f5ebb`。**DBの変更はなし。**
- 配備後（未ログイン）: 公開画面・`app.js`・`map-shell.js`・`owner-route.js`・`story.js`・`issue-ui.css` が200、未認証 `/api/private/summary` は401。件数は `disabled` のまま、`timeline`/`money`/`trips` は登録されない（ゲストには無い画面）。
- **未確認**: ログインが要る所有者側の画面（記録一覧・おかね・旅の実操作、再生の継続/停止）。自動操作のブラウザは未ログインのため通していない。
...
## タブ復帰時の再取得を間引く（Issue #17、2026-09-21）

本番の実測では1回あたりの読み取りは既に軽い（公開フィード3,657行、個人用3,842行。`0009-indexes.sql` により事故当時の約20万行から55分の1）。負荷は頻度側にあり、タブ復帰のたびに全件を読み直していた。ログイン中は個人用フィードになるため30秒エッジキャッシュが効かず、タブの行き来だけで無料枠500万行/日を約1,300往復で使い切る計算だった。

`reload({maxAge})` を追加し、手元のフィードがその新しさなら再取得しない。使うのはタブ復帰だけ（60秒）で、保存・ミュート・旅モード切替・再生は従来どおり必ず読み直す。判定はリクエスト開始前に行うため進行中の取得を取り消さず、取得失敗状態では再利用しない。

代償はタブ復帰直後に最大60秒前のフィードを表示しうること。DBの変更はなく、Worker Version `b8d0ed2c-3668-45f8-b0ff-2eadbd327f60` を配備した。復旧先は `41d39685-e9ad-480f-a9f1-51a21ee8e75f`。

本番確認: タブ復帰3回でフィード取得0回、明示 `reload()` で1回、`maxAge:0` で1回。

差分取得は見送った。`public_entries` に更新日時の列がなく、公開停止は行が減るだけで増分に現れず、旅モード切替は行単位の変更ではない。取りこぼしても画面は正常に見えるため遅延と違い気づけない。詳細はIssue #17。

## 記録フォームの自動測位（Issue #18、2026-09-21）

位置の取り方が入口で割れていた。記録画面 `/admin/record/` は開いた時点で `locate()` が走るのに、地図上のクイック追加フォームは「現在地を入れる」を押さないと座標が入らず、押し忘れると座標なしで保存できた。

クイック追加フォームを開いた時点で測位を始めるよう揃えた。測るのはフォームを開いている間だけで、地図を見ているだけではGPSを掴まない。手で座標を直した場合はその値を上書きせず測位を止める。ボタンは「現在地を入れ直す」で、押すと測位側へ主導権が戻る。保存でフォームがリセットされたら取り直す。取得できない場合は座標を入れず、状態を `#locate-state` に表示する。

見せ方の「場所名だけ」「場所も位置も出さない」は座標を持ったうえで公開側で伏せる別機能で、変更していない。

Worker Version `51853f77-f5bc-4ef8-9b63-9c01abefcfc7` を配備。復旧先は `b8d0ed2c-3668-45f8-b0ff-2eadbd327f60`。DBの変更はなし。

本番確認: フォームを開くとボタンを押さずに `位置を取得中…` へ変わり、測位が未解決の間は座標を入れない。閉じると測位を止める。成功・失敗の各経路はブラウザテストのGPS実装で担保しており、この環境の実機測位では確定しなかった。
