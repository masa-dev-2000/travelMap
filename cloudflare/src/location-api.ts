...
const LEASE_MS=90000,MAX_BYTES=4096;
  ...
  try{
    ...
    if(path==='/api/private/location-capture'&&request.method==='POST'){
      ...
      if(body.command==='start'){
        const previous=body.previous_page_id==null?page:uuid(body.previous_page_id,'引継ぎID');
        ...
        result=await db.prepare(`INSERT INTO location_capture_leases(user_id,client_id,capture_id,page_id,expires_at) VALUES(?,?,?,?,?)
          ...
          WHERE location_capture_leases.expires_at<=? OR (location_capture_leases.capture_id=? AND location_capture_leases.page_id=?) OR (location_capture_leases.capture_id=? AND location_capture_leases.page_id=?)`)
          .bind(uid,client,capture,page,now+LEASE_MS,now,previousCapture,previous,capture,page).run();
      }else if(body.command==='renew'){
        result=await db.prepare('UPDATE location_capture_leases SET expires_at=? WHERE user_id=? AND client_id=? AND capture_id=? AND page_id=? AND expires_at>?').bind(now+LEASE_MS,uid,client,capture,page,now).run();
    ...
    if(path==='/api/private/location-samples'&&request.method==='POST'){
      ...
      const inserted=await db.prepare(`INSERT OR IGNORE INTO location_samples(user_id,id,client_id,capture_id,segment_id,captured_at,received_at,latitude,longitude,accuracy)
        SELECT ?,?,?,?,?,?,?,?,?,? WHERE EXISTS (SELECT 1 FROM location_capture_leases WHERE user_id=? AND client_id=? AND capture_id=? AND page_id=? AND expires_at>?)`)
    ...
    if(path==='/api/private/location-samples'&&request.method==='GET'){
      ...
      if(cursor){
        ...
        const date=stamp(parsed[0]),key=uuid(parsed[1],'続きのID');after=' AND (captured_at<? OR (captured_at=? AND id<?))';params.push(date,date,key);
      }
      params.push(limit);
    ...
    if(error instanceof Error&&/no such table.*location_/i.test(error.message))return reply({error:'自動位置記録はまだ利用できません',code:'location_not_ready'},503);
    throw error;
  }
}
